...
# Called with all flags after the subcommand word already stripped, e.g.:
#   cmd_setting_passthrough -port 443 -username foo
...
main() {
    ...
    case "$cmd" in
        ...
        status)              cmd_status ;;
        settings|setting)
            shift  # drop the subcommand word; remaining $@ are the flags
            if [[ $# -eq 0 ]]; then
                cmd_settings          # no flags → read-only display
            else
                require_root
                cmd_setting_passthrough "$@"   # flags present → write + redisplay
            fi
            ;;
        uri)                 cmd_uri ;;
        log)                 cmd_log ;;
