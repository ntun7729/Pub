...
install_certbot_deps() {
    if command -v apt-get &>/dev/null; then
        ...
        apt-get update -qq
        apt-get install -y -q             -o Dpkg::Options::="--force-confdef"             -o Dpkg::Options::="--force-confold"             tzdata certbot cron || true
    elif command -v yum &>/dev/null; then
        yum install -y -q certbot cronie tzdata || yum install -y -q certbot cron tzdata || true
    elif command -v dnf &>/dev/null; then
        dnf install -y -q certbot cronie tzdata || dnf install -y -q certbot cron tzdata || true
    elif command -v pacman &>/dev/null; then
        pacman -Sy --noconfirm certbot cronie tzdata || true
    elif command -v zypper &>/dev/null; then
        zypper -q install -y certbot cron timezone || true
    else
...
cmd_certbot_issue() {
    ...
    [[ "$domain" =~ ^[A-Za-z0-9.-]+$ ]] || die "Domain must be a plain hostname like example.com — no spaces, comments, or parentheses."
    [[ "$email" =~ ^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$ ]] || die "Email address looks invalid: $email"
